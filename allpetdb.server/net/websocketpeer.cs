...
namespace AllpetDB.Server
{
    ...
    public partial class websockerPeer : allpetchain.httpserver.httpserver.IWebSocketPeer
    {
        ...
        public async Task OnSnapshotEXT_GetBlock(NetMessage msgRecv, byte[] id)
        {
            ...
            try
            {
                UInt64 snapheight = BitConverter.ToUInt64(msgRecv.Params["snapheight"]);
                var snap = peerSnapshots[snapheight];
                byte[] data = snap.GetValueData(AllpetDB.systemtable_block, msgRecv.Params["blockid"]);
                msg.Params["data"] = data;
            }
            catch (Exception err)
            {
